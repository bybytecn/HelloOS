...
#define MAX_ALLOC_BLOCK_NUM 128 * 2
#define TOHM(a) ((uint32_t)a + 0xc0000000) // 0-4M的内存转换到高1G上，否则会读取错误
#define TOPY(a) ((uint32_t)a - 0xc0000000)
...
#define VM_START_ADDR 0x10000000 // 256M
#define VM_CONTROLL_BLOCK_START 0x10000
#define VM_SIZE 0x40000000 // 1G
/**
...
struct vm_alloc_block_t
{
    uint32_t start;
    uint32_t size;
    struct vm_alloc_block_t *next;
};
struct vm_free_block_t
{
    uint32_t start;
    uint32_t size;
    struct vm_free_block_t *next;
};
struct vm_controll_t
{
    struct vm_alloc_block_t *alloc_block;
    struct vm_free_block_t *free_block;
};
